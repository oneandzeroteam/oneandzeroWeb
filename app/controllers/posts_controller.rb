class PostsController < ApplicationController
  ...
  def new
    if user_signed_in?
      @board = Board.where(name: params[:boardname]).first
      @post = Post.new
      @attachments = @post.attachments.build #AttachmentUploader
    else
  ...
  def create
    ...
    @post.attachments << Attachment.create(file: params[:image], post_id: @post.id)
    respond_to do |format|
      if @post.save
        #params[:post_attachments]['image'].each do |i| #AttachmentUploader
